...
class Article {
  ...
  Article({
    ...
    required this.pubDate, required this.description, required this.source,
...
class _NewsDashboardState extends State<NewsDashboard> {
  ...
  void _handleSearch(String q) {
    setState(() {
      _displayList = _allArticles.where((a) => 
        a.title.toLowerCase().contains(q.toLowerCase()) || 
        a.source.toLowerCase().contains(q.toLowerCase())).toList();
    });
  }
  ...
  String _formatDate(String d) {
    try {
      DateTime postDate = DateTime.parse(d);
      Duration diff = DateTime.now().difference(postDate);
      String formatted = DateFormat('dd/MM/yyyy').format(postDate);
      if (diff.inDays <= 3) {
        String rel = diff.inHours < 1 ? "${diff.inMinutes}m ago" : (diff.inHours < 24 ? "${diff.inHours}h ago" : "${diff.inDays}d ago");
    ...
    return Column(
      children: [
        Container(
          ...
          child: const Text("THIS WEBSITE IS STILL IN BETA — DEVELOPMENT IN PROGRESS", 
            textAlign: TextAlign.center, style: TextStyle(color: AppColors.appBackground, fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: 2)),
        ),
        Container(
          ...
          child: Center(
            child: Container(
              ...
              child: Row(
                children: [
                  GestureDetector(onTap: () { setState(() => _activeFilter = "ALL"); _applyLogic(); }, child: Text(width > 500 ? "THE RADICAL" : "TR", style: GoogleFonts.spaceGrotesk(color: widget.primaryColor, fontSize: 24, fontWeight: FontWeight.bold, letterSpacing: -1))),
        ...
        children: [
          SizedBox(
            ...
            child: PageView.builder(
              controller: _heroController, onPageChanged: (i) => setState(() => _heroIndex = i),
              itemCount: items.length, itemBuilder: (c, i) => _heroTile(items[i]),
            ),
          ),
          if (width > 1000) ...[
        ...
        child: Stack(
          children: [
            ...
            Positioned(bottom: 40, left: 40, right: 40, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text("${a.source} • ${_formatDate(a.pubDate)}".toUpperCase(), style: const TextStyle(fontSize: 10, color: AppColors.textMuted, fontWeight: FontWeight.bold)),
              const SizedBox(height: 12),
              Text(a.title, style: GoogleFonts.spaceGrotesk(fontSize: 32, fontWeight: FontWeight.bold, fontStyle: FontStyle.italic)),
            ])),
          ],
        ),
      ),
          ...
          children: [
            ...
            Padding(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(_formatDate(a.pubDate).toUpperCase(), style: const TextStyle(fontSize: 9, color: AppColors.textSubtle, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 8),
                  if (a.topics.isNotEmpty) ...[Wrap(spacing: 4, runSpacing: 4, children: a.topics.map((t) => _badge(t, AppColors.highlightOverlay, AppColors.textMain)).toList()), const SizedBox(height: 8)],
                  Text(a.source, style: TextStyle(color: widget.primaryColor, fontSize: 10, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 8),
                  SizedBox(height: 48, child: Text(a.title, maxLines: 2, overflow: TextOverflow.ellipsis, style: GoogleFonts.spaceGrotesk(fontSize: 18, fontWeight: FontWeight.bold))),
                  const SizedBox(height: 8),
                  SizedBox(height: 40, child: Text(a.description, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: AppColors.textMuted, fontSize: 13))),
                ],
              ),
            )
          ],
