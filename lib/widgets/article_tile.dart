...
class _ArticleTileState extends State<ArticleTile> {
  ...
  void _extractVibrantColor() async {
    ...
    try {
      final pg = await PaletteGenerator.fromImageProvider(
        NetworkImage(widget.article.thumbnail),
      );
      if (mounted) {
        setState(() => _vibrantColor = pg.vibrantColor?.color ?? pg.dominantColor?.color);
      }
    } catch (_) {}
  }
            ...
            child: ClipRRect(
              ...
              child: Stack(
                children: [
                  ...
                  if (width > 500) _buildDesktopArrows(),
                ],
              ),
            ),
          ),
      ...
      child: Container(
        ...
        child: Stack(
          children: [
            ...
            Positioned(top: 12, left: 12, child: Wrap(spacing: 4, runSpacing: 4, children: widget.article.topics.map((t) => badge(t, Colors.white, Colors.black)).toList())),
  ...
  Widget _buildInfoSlide(String fullDesc) {
    return Container(
      ...
      decoration: BoxDecoration(
        // FIXED: Blended vibrant color with black to darken it and make text stand out
        color: Color.lerp(_vibrantColor, Colors.black, 0.6)?.withValues(alpha: 0.95) ?? AppColors.tileBackground, 
        borderRadius: BorderRadius.circular(28), 
        border: Border.all(color: AppColors.appBackground),
      ),
      child: Column(
  ...
  Widget _buildPaginationDots() {
    return Positioned(bottom: 12, left: 0, right: 0, child: Row(mainAxisAlignment: MainAxisAlignment.center, children: List.generate(2, (index) => Padding(padding: const EdgeInsets.symmetric(horizontal: 2), child: Icon(FontAwesomeIcons.circle, size: 6, color: _currentIndex == index ? Colors.white : Colors.white24)))));
  }
...
Widget badge(String t, Color bg, Color tc) => Container(
  padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), color: bg, 
